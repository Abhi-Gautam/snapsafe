...
fn test_global_config_creation() {
    ...
    let before = Command::cargo_bin("snapsafe")
        .unwrap()
    ...
    Command::cargo_bin("snapsafe")
        ...
        .args([
            ...
            "rs,toml,md,TEST_MARKER",
        ])
        .assert()
        .success();

    // Immediately get the value back to verify
    let output = Command::cargo_bin("snapsafe")
        .unwrap()
    ...
    assert!(
        output_str.contains("TEST_MARKER"),
        "Expected to find TEST_MARKER in output: {}",
        output_str
    );
...
fn test_snapshot_with_custom_ignore() {
    ...
    fs::write(
        temp_path.join("custom_ignore_dir").join("ignored.txt"),
