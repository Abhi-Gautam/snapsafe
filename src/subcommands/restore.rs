...
pub fn restore_snapshot(snapshot_id: Option<String>, backup: bool) -> io::Result<()> {
    ...
    if head_manifest.is_empty() {
        return Err(io::Error::new(ErrorKind::NotFound, "No snapshots available to restore."));
    }
                ...
                match exact_match {
                    ...
                    None => {
                        head_manifest.iter()
                            ...
                            .map(|s| s.version.clone())
                            .ok_or_else(|| {
                                io::Error::new(ErrorKind::NotFound, format!("Snapshot {} not found", id))
                            })?
                    }
                }
            }
    ...
    let (_, manifest) = snap_option.ok_or_else(|| {
        io::Error::new(ErrorKind::NotFound, format!("Manifest for snapshot {} not found", version))
    })?;
    
    // Get the snapshot info from head manifest for display
    let snapshot_info = head_manifest.iter()
